...
const labTitles: Record<string, string> = {
  lab1: "Reflected XSS in a search field",
  lab2: "Stored XSS in a comment box",
};

export default function XssLabPage() {
  const params = useParams<{ labId: string }>();
  const labId = params.labId;
  const solved = useModuleLabSolved("xss", labId);
  const title = labTitles[labId] || "XSS practice lab";
  ...
  return (
    <div className="min-h-screen bg-white px-6 py-10 text-black">
      <main className="mx-auto max-w-3xl rounded-lg border border-gray-200 p-8 shadow-sm">
        <Link href="/learn/xss/lablist" className="mb-6 inline-block text-sm font-semibold text-gray-500 hover:text-black">
          Back to XSS labs
        </Link>
        ...
        <button
          onClick={() => markModuleLabSolved("xss", labId)}
