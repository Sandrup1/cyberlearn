...
import { defaultCourseContents, CourseContent } from "../../lib/course-content";
...
export async function GET() {
  try {
    ...
    const dbCourses = await db.collection<CourseContent>("courseContents").find({ published: true }).toArray();
    
    // Merge DB courses with default ones. DB overrides default ones if same moduleId
    const mergedCourses: Record<string, CourseContent> = { ...defaultCourseContents };
    
    for (const course of dbCourses) {
      // Omit _id if it exists
      const { _id, ...courseData } = course as any;
