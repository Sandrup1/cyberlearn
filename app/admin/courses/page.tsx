...
function cloneContent(content: CourseContent): CourseContent {
  return JSON.parse(JSON.stringify(content)) as CourseContent;
}

function emptySection(id: string): ContentSection {
  return {
    id,
    label: "New section",
    heading: "New section",
...
function cardToneLabel(tone: ContentCard["tone"]) {
  return tone === "green"
    ? "Green"
    : tone === "blue"
    ? "Blue"
...
export default function AdminCourses() {
  const [contents, setContents] = useState<Record<CourseModuleId, CourseContent>>(
    () => {
      const initial: Record<CourseModuleId, CourseContent> =
        {} as Record<CourseModuleId, CourseContent>;
      Object.values(defaultCourseContents).forEach((content) => {
        initial[content.moduleId as CourseModuleId] = cloneContent(content);
      });
      return initial;
    }
  );

  const [selectedModuleId, setSelectedModuleId] = useState<CourseModuleId>("sqli");
  const [activeSectionId, setActiveSectionId] = useState<string>("");

  const [draft, setDraft] = useState<CourseContent | null>(() => {
  ...
  const moduleList = useMemo(() => {
    return Object.keys(contents).sort() as CourseModuleId[];
  }, [contents]);
  ...
  useEffect(() => {
    ...
    async function loadContents() {
      ...
      try {
        ...
        const data = (await res.json()) as CourseContent[];

        if (!active || !res.ok) {
          throw new Error("Failed to load course content");
        }

        const next: Record<CourseModuleId, CourseContent> =
          {} as Record<CourseModuleId, CourseContent>;
        data.forEach((content) => {
          next[content.moduleId as CourseModuleId] = cloneContent(content);
        });

        setContents(next);
        setLoadState("ready");
        setStatus("Saved content loaded.");
        const firstModuleId = data[0]?.moduleId as CourseModuleId | undefined;
        if (firstModuleId) {
        ...
        const fallbackFirst = fallbackIds[0] as CourseModuleId | undefined;
        if (fallbackFirst) {
  ...
  async function createModule() {
    ...
    if (contents[moduleId as CourseModuleId]) {
      setStatus("A module with that id already exists.");
      return;
    }

    const now = new Date().toISOString();
    const payload: CourseContent = {
      moduleId,
    ...
    try {
      ...
      setContents((prev) => ({
        ...prev,
        [moduleId as CourseModuleId]: cloneContent(payload),
      }));
      setSelectedModuleId(moduleId as CourseModuleId);
      setCreateOpen(false);
      setCreateModuleId("");
      setCreateTitle("");
      setCreateShortTitle("");
  ...
  function updateDraft(patch: Partial<CourseContent>) {
    if (!draft) return;
    setDraft({ ...draft, ...patch });
  }

  function updateSection(sectionId: string, patch: Partial<ContentSection>) {
    if (!draft) return;
  ...
  async function save() {
    ...
    try {
      ...
      setContents((current) => ({
        ...current,
        [draft.moduleId]: cloneContent(data.content as CourseContent),
      }));
              ...
              <div className="mt-4 space-y-2">
                {moduleList.length === 0 ? (
                  ...
                  moduleList.map((moduleId) => {
                    const content = contents[moduleId as CourseModuleId];
                    const selected = selectedModuleId === moduleId;
                  return (
                    <button
                      key={moduleId}
                      type="button"
                      onClick={() => setSelectedModuleId(moduleId as CourseModuleId)}
                                ...
                                <div className="mb-2 grid grid-cols-1 gap-2 sm:grid-cols-2">
                                  ...
                                  <select
                                    ...
                                    onChange={(event) => {
                                      ...
                                      next[idx] = {
                                        ...next[idx],
                                        tone: event.target.value as ContentCard["tone"],
                                      };
                                      updateSection(activeSection.id, { cards: next });
                                    }}
                                    className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-600 focus:ring-2 focus:ring-blue-100"
                                  >
                                    {(["green", "blue", "amber", "red"] as const).map((tone) => (
