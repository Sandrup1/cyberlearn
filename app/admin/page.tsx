  ...
  useEffect(() => {
    ...
    async function loadStats() {
      ...
      try {
        ...
        const usersJson = (await usersRes.json()) as unknown;
        const contentJson = (await contentRes.json()) as unknown;

        if (!active) return;

        const users = Array.isArray(usersJson) ? usersJson : [];
        const modules = Array.isArray(contentJson) ? (contentJson as CourseContent[]) : [];
